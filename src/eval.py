...
def eval_bpc(model: PreTrainedModel, tokenizer: PreTrainedTokenizerFast, dataset: Dataset,
        dataset_size: int=None) -> tuple[float, int]:
    ...
    for example in tqdm(dataset, total=dataset_size):
        ...
        inputs = tokenizer(example["text"], return_tensors="pt", truncation=True, max_length=512)  # This is a 1 x n_tokens batch.
